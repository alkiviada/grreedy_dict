...
def scrape_wordref_words(words_string, split=1):
  ...
  if re.match('Note', words_string):
    return words_string
  words_string = re.sub(
    r'(?<!^)(?<!\[)(?<!\:\ )\b(ab(b)?r$|inter$|(proper )?n(m|f|noun|pl|pr| propre|m(f)?nom)?\b|pp|prépp|s$|hjv\b|uttr\b|vbal\b|'
     'préf$|prefix|suffix|v(i|tr)?$|v(i|tr(verbe)?)?( ?(\+ )?(impers|indverbe|rif|refl|prép|past|aux|pron|pres|phras|expr|(in)?trans).*?)$|Note|'
     'loc (.+)|loc$|agg$|adj((( inv)?adj.+| inv| n)?.+?)?$|interj(interj.+?)?$|adv(adv.+)?$|avv$| contraction|expr((expr|verb).*)?$|'
