...
urlpatterns = [
    ...
    url(r'^homework/conjugations/(?P<word>[-\'\w() ]+)/(?P<language>(french|italian))/(?P<tense_idx>[0-9]+)$', 
      api_views.Conjugate.as_view(), name='verb-conjugations'),
    url(r'^homework/tenses/(?P<word>[-\'\w() ]+)/(?P<language>(french|italian))$', api_views.VerbTenses.as_view(), name='verb-tenses'),
